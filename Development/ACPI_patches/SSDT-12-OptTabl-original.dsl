 ...
 *
 * Disassembly of SSDT-12-OptTabl.aml, Sun Jul 21 05:12:11 2019
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x000022B7 (8887)
 *     Revision         0x01
 *     Checksum         0xC3
 *     OEM ID           "OptRef"
...
DefinitionBlock ("", "SSDT", 1, "OptRef", "OptTabl", 0x00001000)
{
    ...
    External (_SB_.PCI0.LPCB.EC0_.SKID, UnknownObj)    // (from opcode)
    ...
    External (_SB_.PCI0.PEG0.NVDV, IntObj)    // (from opcode)
    ...
    Scope (\_SB.PCI0.PEG0.PEGP)
    {
        ...
        Method (_ROM, 2, NotSerialized)  // _ROM: Read-Only Memory
        {
            ...
            ElseIf (LLess (Local0, 0x00038000))
            {
                Subtract (Local0, 0x00030000, Local0)
                Store (RBF7, ROM1)
            }
            ElseIf (LLess (Local0, 0x00040000))
            {
                Subtract (Local0, 0x00038000, Local0)
                Store (RBF8, ROM1)
            }
    ...
    Scope (\_SB.PCI0.PEG0.PEGP)
    {
        ...
        {
            If (LEqual (\_SB.GGIV (0x03070012), One))
            {
        ...
        Method (GPS, 4, Serialized)
        {
            ...
            Switch (ToInteger (Arg2))
            {
                Case (Zero)
                {
                    ...
                    If (LEqual (\_SB.PCI0.LPCB.EC0.SKID, 0x04))
                    {
                        Return (Buffer (0x08)
                        {
                ...
                Case (0x22)
                {
                    ...
                    If (LLess (PCAP, \_SB.PCI0.LPCB.EC0.ECE1))
                    {
                        Store (\_SB.PCI0.LPCB.EC0.ECE1, PCAP)
                    }
                ...
                Case (0x2A)
                {
                    ...
                    Switch (ToInteger (PSH0))
                    {
                        ...
                        {
                            Store (0x0102, RETN)
                            If (LAnd (CondRefOf (\_SB.PCI0.LPCB.EC0.NDTT), LNotEqual (\_SB.PCI0.LPCB.EC0.ECUP, One)))
                            {
                                Store (\_SB.PCI0.LPCB.EC0.NDTT, TGPU)
                            }
                            Else
                            {
                                Store (Zero, TGPU)
